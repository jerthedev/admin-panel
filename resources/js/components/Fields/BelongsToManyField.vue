      ...
      <div v-show="!isCollapsed">
        ...
        >
          <div class="text-gray-500" :class="{ 'text-gray-400': isDarkTheme }">
            ...
            <p class="text-sm">
              {{ field.showCreateRelationButton ? 'Attach existing items or create new ones to get started.' : 'Attach existing items to get started.' }}
            </p>
          </div>
        </div>
            ...
            <div class="flex items-center justify-between">
              <div class="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <div v-if="item.pivot && field.pivotFields.length > 0" class="mt-2">
                    <div class="text-xs text-gray-500" :class="{ 'text-gray-400': isDarkTheme }">
                      Pivot data: {{ formatPivotData(item.pivot) }}
                    </div>
                  </div>
                </div>
              </div>
 ...
 * 
 * Many-to-many relationship field with attach/detach, pivot fields, and management capabilities.
 * Displays multiple related models with full CRUD operations and pivot table support.
 * 
...
const props = defineProps({
  ...
  modelValue: {
    type: Object,
    default: () => ({ count: 0, resource_id: null, resource_class: null, pivot_fields: [], pivot_computed_fields: [], pivot_actions: [] })
  },
  errors: {
    type: Object,
    default: () => ({})
